...
describe('StepSpells - Specific Behavior', () => {
  ...
  describe('Spell Toggle Selection', () => {
    ...
    it('adds spell to selection when not selected', async () => {
      ...
      expect(vm.selectedSpells.get('test-choice')?.has('1')).toBe(true)
    })
    ...
    it('removes spell from selection when already selected', async () => {
      ...
      expect(vm.selectedSpells.get('test-choice')?.has('1')).toBe(false)
    })
    ...
    it('respects quantity limit', async () => {
      ...
      vm.handleSpellToggle(mockChoice, { id: 1, name: 'Spell 1' })
      vm.handleSpellToggle(mockChoice, { id: 2, name: 'Spell 2' })
      // Try to add beyond limit
      vm.handleSpellToggle(mockChoice, { id: 3, name: 'Spell 3' })
      await wrapper.vm.$nextTick()
